...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  async function deployD3Oracle(shouldSet: boolean) {
    ...
    if (shouldSet) {
      ...
      sleep(10);
      const priceSourceWMATIC = {
        oracle: config.chainlinkPriceFeed.MATICUSD,
        isWhitelisted: true,
        priceTolerance: BigNumber.from(padZeros(9, 17)),
        priceDecimal: 8,
        tokenDecimal: 18,
      ...
      console.log("setPrciceSource for WMATIC...");
      await D3Oracle.setPriceSource(
  ...
  async function deployD3Proxy() {
    ...
    const wmatic = config.defaultAddress.WMATIC;
    console.log("wmatic", wmatic);
    await deployContract("D3Proxy", "D3Proxy", [
      dodoApproveProxy,
      wmatic,
      vault,
    ]);
