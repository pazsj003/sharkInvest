...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  async function deployD3Oracle(shouldSet: boolean) {
    ...
    if (shouldSet) {
      ...
      await D3Oracle.setPriceSource(config.defaultAddress.ETH, priceSourceETH);

      const priceSourceBNB = {
        oracle: config.chainlinkPriceFeed.BNB_USD,
        isWhitelisted: true,
        priceTolerance: BigNumber.from(padZeros(9, 17)),
        priceDecimal: 8,
        tokenDecimal: 18,
      ...
      console.log("setPriceSource for WBNB...");
      await D3Oracle.setPriceSource(config.defaultAddress.WBNB, priceSourceBNB);
  ...
  async function deployD3Proxy() {
    ...
    const wbnb = config.defaultAddress.WBNB;
    console.log("wbnb", wbnb);
    await deployContract("D3Proxy", "D3Proxy", [dodoApproveProxy, wbnb, vault]);
  }

  async function deployD3UserQuota() {
    const Dodo = config.defaultAddress.DODO;
    const vault = config.deployedAddress.D3Vault;
