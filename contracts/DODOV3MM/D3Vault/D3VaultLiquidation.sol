...
contract D3VaultLiquidation is D3VaultFunding {
    ...
    function isPositiveNetWorthAsset(
        address pool,
        address token
    ...
    function getPositiveNetWorthAsset(
        address pool,
        address token
    ...
    function liquidate(
        ...
        if (ID3MM(pool).isInLiquidation())
            revert Errors.D3VaultAlreadyInLiquidation();
        if (checkBadDebtAfterAccrue(pool)) revert Errors.D3VaultHasBadDebt();
        if (!checkCanBeLiquidatedAfterAccrue(pool))
            revert Errors.D3VaultCannotBeLiquidated();
        if (!isPositiveNetWorthAsset(pool, collateral))
            revert Errors.D3VaultInvalidCollateralToken();
        if (isPositiveNetWorthAsset(pool, debt))
            revert Errors.D3VaultInvalidDebtToken();
        if (getPositiveNetWorthAsset(pool, collateral) < collateralAmount)
        ...
        uint256 collateralAmountMax = debtToCover.mul(debtTokenPrice).div(
            collateralTokenPrice.mul(DISCOUNT)
        );
        if (collateralAmount > collateralAmountMax)
            revert Errors.D3VaultCollateralAmountExceed();
    ...
    function startLiquidation(
        ...
        for (uint256 i; i < tokenList.length; i++) {
            ...
            BorrowRecord storage record = info.borrowRecord[pool];
            uint256 debt = _borrowAmount(
                record.amount,
                record.interestIndex,
                info.borrowIndex
            ).mul(ratio); // borrowAmount = record.amount * newIndex / oldIndex
    ...
    function liquidateByDODO(
        ...
        if (!ID3MM(pool).isInLiquidation())
            revert Errors.D3VaultNotInLiquidation();

        uint256 toTokenReserve = IERC20(order.toToken).balanceOf(address(this));
        uint256 fromTokenValue = DecimalMath.mul(
            ID3Oracle(_ORACLE_).getPrice(order.fromToken),
            order.fromAmount
        );

        // swap using Route
        {
            IERC20(order.fromToken).safeTransferFrom(
                pool,
                router,
                order.fromAmount
            );
        ...
        uint256 toTokenValue = DecimalMath.mul(
            ID3Oracle(_ORACLE_).getPrice(order.toToken),
            receivedToToken
        );

        if (toTokenValue < fromTokenValue.mul(DISCOUNT))
            revert Errors.D3VaultExceedDiscount();
    ...
    function finishLiquidation(
        ...
        for (uint256 i; i < tokenList.length; i++) {
            ...
            uint256 realDebt = _borrowAmount(
                borrows,
                record.interestIndex,
                info.borrowIndex
            ); // borrowAmount = record.amount * newIndex / oldIndex
